...
class IntracranialDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if torch.argmax(labels)==0:
            ...
            mask = cv2.imread(img_name.replace('_img.png', '_annotation.png'), 0)
...
def multi_acc(y_pred, y_test):
    ...
    return [int(trueval),len(correct_pred)]
...
def train_phase(net,trnloader,scheduler,optimizer,criterion,valloader):
    for epoch in range(n_epochs):
        ...
        tr_acc = tr_acc / data_num
        print('Train loss: {}, Train acc: {}'.format(round(tr_loss, 4), round(tr_acc, 4), ))
